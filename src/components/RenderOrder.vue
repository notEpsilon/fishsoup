<template>
    <div class="max-w-sm w-full lg:max-w-full lg:flex shadow">
        ...
        <div class="w-full border-r border-b border-l lg:border-l-0 lg:border-t bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal" :class="ord.status === false ? 'border-teal-400 lg:border-teal-400' : 'border-purple-400 lg:border-purple-400'">
            <div class="mb-8 flex justify-between">
                ...
                <button v-if="worker && ord.status !== true" @click="markAsDone" class="rounded-md py-2 w-32 lg:w-36 bg--500 h-[2.32rem] bg-rose-600 hover:bg-rose-500 text-white align-middle text-center text-sm">
                    Mark as Done
                </button>
            </div>
            <div class="flex items-center justify-between">
                ...
                <badge-component :text="ord.status === false ? 'In Progress' : 'Done'" :done="ord.status === false ? false : true" />
            </div>
        </div>
    </div>
</template>
